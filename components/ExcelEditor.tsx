...
function reducer(state: WorkbookState, action: Action): WorkbookState {
  ...
  switch (action.type) {
    ...
    case "NEXT_COLUMN": {
      const width = colWidth(state.grid);
      const nextCol = Math.min(state.activeCol + 1, width);
      return { ...state, activeCol: nextCol };
    }
    case "FILL_VALUE": {
      ...
      if (action.advance === "column") {
        const nextCol = Math.min(activeCol + 1, width);
        if (g.length > maxTotalLen) g = g.slice(0, maxTotalLen);
        return {
          ...state,
          grid: g,
      ...
      const nextActive = Math.min(activeRow + 1, maxExcelRow);
      while (g.length < nextActive) {
        g = [...g, Array(width).fill(null)];
      }
      if (g.length > maxTotalLen) g = g.slice(0, maxTotalLen);
      return {
...
export function ExcelEditor({ workbookId, workbookName }: Props) {
  ...
  return (
    <main className="mx-auto flex h-[100dvh] max-h-[100dvh] w-full max-w-6xl flex-col overflow-hidden px-3 py-2">
        ...
        >
          ...
          <div className="shrink-0 space-y-2 rounded-lg border border-slate-700/80 bg-slate-950/40 p-3">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <div className="flex min-w-[8rem] flex-1 flex-col gap-1 sm:min-w-[12rem]">
